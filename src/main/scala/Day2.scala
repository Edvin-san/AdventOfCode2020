...
object Day2 extends Day[Long, Long] {
  ...
  object Rule {
    def fromLine(s: String): Rule = {
      val low = s.takeWhile(_ != '-').toInt
      val high = s.dropWhile(_ != '-').drop(1).takeWhile(_ != ' ').toInt
      val c = s.split(":").head.last
      val password = s.split(" ").last
      Rule(low, high, c, password)
    }
  }

  def firstPolicy(r: Rule) = {
    val occurrences = r.password.filter(_ == r.c).size
    r.a <= occurrences && occurrences <= r.b
  }

  def checkOrdinal(s: String, i: Int): IO[NoSuchElementException, Char] = IO.effect(s.charAt(i - 1)).refineToOrDie[NoSuchElementException]

  def secondPolicy(r: Rule): UIO[Boolean] = for {
    c1 <- checkOrdinal(r.password, r.a).option
    c2 <- checkOrdinal(r.password, r.b).option
  } yield c1.contains(r.c) ^ c2.contains(r.c)

  def part1(in: String) = UIO(Source.fromString(in).getLines().map(Rule.fromLine).filter(firstPolicy).size)
