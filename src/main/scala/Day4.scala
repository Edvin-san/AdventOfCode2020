...
object Day4 extends Day[Long, Long] {
  type Passport = Map[String, String]

  def parsePassports(s: String): List[Passport] = (for {

    passportS <- {
      if (s == null) println("What the hell");
      s.split("\n\n")
    }
    p = for {
      kv <- passportS.replace("\n", " ").split(" ")
      x = kv.split(":")
    } yield (x(0), x(1))
  } yield p.toMap).toList

  def isPresent(p: Passport): Boolean = List("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid").map(p.contains).reduce(_ && _)

  def between(low: Int, high: Int)(v: Int): Boolean = v >= low && v <= high

  def isValid(key: String, value: String) = (key match {
    case "byr" => Try(value.toInt).map(between(1920, 2002))
    case "iyr" => Try(value.toInt).map(between(2010, 2020))
    case "eyr" => Try(value.toInt).map(between(2020, 2030))
    case "hgt" => value.splitAt(value.size - 2) match {
      case (digits, "cm") => Try(digits.toInt).map(between(150, 193))
      case (digits, "in") => Try(digits.toInt).map(between(59, 76))
      case _ => Failure(new Throwable("Not cm or in."))
    }
    case "hcl" => value.splitAt(1) match {
      case ("#", chars) if chars.size == 6 => Success(chars.forall(c => c.isDigit || c.toString.matches("[a-f]")))
      case _ => Failure(new Throwable("No # or not exactly six characters after."))
    }
    case "ecl" => Success(Set("amb", "blu", "brn", "gry", "grn", "hzl", "oth").contains(value))
    case "pid" => Success(value.size == 9 && value.forall(_.isDigit))
    case _ => Success(true)
  }).getOrElse(false)

  def part1(in: String) = UIO {
    val passports = parsePassports(in)
    passports.filter(isPresent).size
  }

  def part2(in: String) = UIO {
    parsePassports(in).filter(isPresent).filter(_.forall((isValid _).tupled)).size
  }
